<template>
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="产品类别" prop="chanpingleibie">
					<el-input v-model="ruleForm.chanpingleibie" placeholder="产品类别" clearable  :readonly="ro.chanpingleibie"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="产品类别" prop="chanpingleibie">
					<el-input v-model="ruleForm.chanpingleibie" placeholder="产品类别" readonly></el-input>
				</el-form-item>
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `chanpingleibie/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.chanpingleibieCrossAddOrUpdateFlag = false;
											this.parent.search();
